...
int mt_dsi_pinctrl_set(unsigned int pin , unsigned int level)
{
	...
	case LCM_POWER_DM_NO: 	
      ...
      case LCM_POWER_DP_NO:   
          if( IS_ERR(lcm_power_dp_en0)|| IS_ERR(lcm_power_dp_en1))
          {
           ...
           break;
      case LCM_POWER_ENN:   
          if( IS_ERR(lcm_power_enn0)|| IS_ERR(lcm_power_enn1))
          {
               pr_err("err: lcm_power_enn0 or lcm_power_enn1 is error!!!");
               return ret;
          }   
          else
           break;
      case LCM_POWER_ENP:   
          if( IS_ERR(lcm_power_enp0)|| IS_ERR(lcm_power_enp1))
          {
               pr_err("err: lcm_power_enp0 or lcm_power_enp1 is error!!!");
               return ret;
          }   
          else
           break;
	...
	case LCM_POWER_ENP: pinctrl_select_state(dsi_pinctrl1, level ? lcm_power_enp1 : lcm_power_enp0); break; 	
	case LCM_POWER_ENN: pinctrl_select_state(dsi_pinctrl1, level ? lcm_power_enn1 : lcm_power_enn0); break; 	
	default: ;
	}
      return 0;
